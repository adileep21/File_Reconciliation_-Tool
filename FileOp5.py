...
if operation == "Append Files":
    ...
    if file:
        ...
        if df is not None:
            ...
            # Dropdown to Select a Column for Sum Calculation
            numeric_columns = df.select_dtypes(include="number").columns.tolist()
            if numeric_columns:
                selected_sum_column = st.selectbox("**Select a column for Grand Total**", numeric_columns)
                if selected_sum_column:
                    column_sum = df[selected_sum_column].sum()
                    st.write(f"The Sum of {selected_sum_column} is {column_sum:.2f}")

            group_by_columns = st.multiselect("**Select columns to group by**", df.columns)
            selected_columns = st.multiselect("**Select numeric column to summarize**", numeric_columns)
            operation = st.selectbox("**Choose an operation**", ["Sum", "Mean", "Median", "Min", "Max", "Count"])
            
            # Checkbox to include all columns in summarized data
            include_all_columns = st.checkbox("Include all columns")
            
            if st.button("Summarize"):
                summary_df = df.groupby(group_by_columns).agg({col: operation.lower() for col in selected_columns})
                
                if include_all_columns:
                    summary_df = df[group_by_columns + selected_columns + [col for col in df.columns if col not in group_by_columns + selected_columns]]
                    
                st.markdown("### 📋 Summarized Data")
                st.dataframe(summary_df)

                st.write(f"**Summarized Data - Rows:** {summary_df.shape[0]}, **Columns:** {summary_df.shape[1]}")
                if selected_columns:
                    total_summary_sum = summary_df[selected_columns].sum().sum()
                    st.write(f"**Total Sum of** {', '.join(selected_columns)}: {total_summary_sum:.2f}")
                
                st.download_button("Download Summarized Data", convert_df_to_excel(summary_df), "summary_data.xlsx")
